...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # Add empty set as new action level A_i
        self.a_levels.append(set())

        # For every action node
        for node in map(PgNode_a, self.all_actions):

            # Filter action with prerequisites that hold in S_i
            if not node.prenodes.issubset(self.s_levels[level]):
                continue

            # Connectt action to all prerequisites
            for n in node.prenodes:
                node.parents.add(n)

            # Add action to action level A_i
    ...
    def add_literal_level(self, level):
        ...
        """
        # Get actions from previous level A_i-1
        actions = self.a_levels[level-1]

        # Get effects for actions from level A_i-1
        effects = map(PgNode_a.effect_s_nodes, actions)

        # Create literal set out of all effect lists
        literals = set(chain(*effects))

        # Add new S_i level set
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Get effect pairs for both action nodes
        pairs = product(*map(PgNode_a.effect_s_nodes, [node_a1, node_a2]))

        # Test mutex for any possible pair of effects
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Test whether node_s1 is a negation of node_s2
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Test whether no pair of actions could achieve node_s1 and node_s2 at the same time
        return all(PgNode.is_mutex(*pair) for pair in product(node_s1.parents, node_s2.parents))
